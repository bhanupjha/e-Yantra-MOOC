...
void select_adc_channel(unsigned char channel_num) {
	// Use bitwise operators to set the MUX[5:0] bits based on the channel number
	if (channel_num > 7) {
	...
	// Clear the previous channel selection bits and set the new channel number
	ADMUX_reg = (ADMUX_reg & ~0x07) | (channel_num & 0x07);
}
...
void reset_adc_config_registers(void){
	// Clear the MUX5 bit for the next conversion
	ADCSRB_reg &= ~(1 << MUX5_bit);

	// Clear the MUX[4:0] bits for the next conversion
	ADMUX_reg &= ~((1 << MUX2_bit) | (1 << MUX1_bit) | (1 << MUX0_bit));

}

// read the ADC data after the conversion is complete
ISR(ADC_vect) {
	// Read the ADC data by first reading ADCL and then ADCH
	adc_10bit_data =  ((ADCL_reg << 8) | ADCH_reg);

	// Reset ADC config registers
...
int main(void) {
	...
	char tx_raw_adc_data_buffer[25], lcd_print_raw_adc_data_string[25];
	
	convert_analog_channel_data( sharp_sensor_channel );
	
	while(1)
	{
		if ( ( ADCSRA & ( 1 << ADSC_bit ) ) != 0x40 )
		{
			// ------------------------------- //
